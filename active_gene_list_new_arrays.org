...
## ## Save results
## outname <- sprintf(
##   'state_df_rna%s_red%s_redresc%s_reddw%s_areaFC%s_redpcntdif%s_allstrains.csv',
##   th_rnapcnt, th_redpcnt, th_redrescue, th_reddown, th_areaFC, th_redpcntdif
## )
## write_tsv(state_df, paste0(outdir, outname))

#+END_SRC
** Create Lists according to thresholds Old
#+begin_src R
print(final_df, width = 200, n = 1000)
...
set_state <- function(df, statecol, redcol, relcol){
  ...
  df <- df %>%
    mutate(!! statecol := case_when(
                ...
                areaFC >= th_areaFC &
                MaxRedPercent >= th_reddown &
                !! relcol == 'min' ~ 'CVG_Repressed_FC',

                Variant &
                areaFC >= th_areaFC &
                MaxRedPercent < th_reddown ~ 'CVG_Repressed_FC_reddown',

                Variant &
                areaFC < th_areaFC &
                MeanRedPercent >= th_redpcnt ~ 'CVG_Active_noFC',

                Variant &
                areaFC < th_areaFC &
                MeanRedPercent < th_redpcnt ~ 'CVG_Repressed_noFC',

                ## ## Exceptions
                ## Variant &
                ## MaxRedPercent < th_redpcnt &
                ## areaFC > th_areaFC_redrescue &
                ## !! relcol == 'max' ~ 'CVG_Semiactive_FC_nored',

                ## Variant &
                ## MaxRedPercent < th_redpcnt &
                ## areaFC > th_areaFC_redrescue &
                ## !! relcol == 'mid' ~ 'CVG_Repressed_FC_nored',

                ## Variant &
                ## MaxRedPercent < th_redpcnt &
                ## areaFC > th_areaFC_redrescue &
                ## !! relcol == 'min' ~ 'CVG_Repressed_FC_nored',
                ...
                Variant &
                is.na(areaFC) &
                MeanRedPercent >= th_redpcnt ~ 'CVG_Active_FCna',

                Variant &
                is.na(areaFC) &
                MeanRedPercent < th_redpcnt ~ 'CVG_Repressed_FCna',
...
set_category <- function(df, statecol, categorycol){
  ...
  df <- df %>%
    mutate(!! categorycol := case_when(
                ...
                startsWith(!! statecol, 'CVG_Semiactive') ~ 'CVG_Semiactive',
                startsWith(!! statecol, 'CVG_Repressed') ~ 'CVG_Repressed',
