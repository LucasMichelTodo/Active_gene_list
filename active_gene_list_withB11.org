...
## Add B11
b11_fld <- '/mnt/Disc4T/Projects/PhD_Project/Microarrays/New_Old_separate_approach/New_Arrays/R_results_NewArray/'

B11_red <- read_csv(paste0(b11_fld, 'geneLevel_redSignal_exp.csv')) %>%
...
## red_percent_old <- read_csv('/mnt/Disc4T/Projects/PhD_Project/Microarrays/New_Old_separate_approach/Old_Arrays/R_results_OldArrays_Variantome/red_percentiles.csv')
## red_percent_new <- read_csv('/mnt/Disc4T/Projects/PhD_Project/Microarrays/New_Old_separate_approach/New_Arrays/R_results_NewArray/red_percentiles.csv')

## red_df <- full_join(red_percent_old, red_percent_new, by = 'Gene_id')
## red_df[red_df$Gene_id == 'PF3D7_0935400_as',]$Gene_id <- 'PF3D7_0935390'

## red_df <- red_df %>%
##   set_names(c('Gene_id', 'Percent_12B', 'Percent_10G', 'Percent_3D7B',
##               'Percent_A7', 'Percent_E5', 'Percent_B11')) %>%
##   select(Gene_id, contains(c('12B', '10G', '3D7B', 'B11')))
...
B11_area <- read_csv(paste0(b11_fld, 'area_geneLevel.csv')) %>%
  select(Gene_id, contains('B11')) %>%
...
## Save results
outname <- sprintf(
  './Results_Tables_withB11/state_df_rna%s_red%s_redresc%s_reddw%s_areaFC%s_redpcntdif%s.csv',
...
write.csv(clags, './Results_Tables_withB11/clag_genes.csv')
...
write.csv(compare_12Bvs10G, './Results_Tables_withB11/gens_dif12B_10G.csv')
#+END_SRC
** Venn Diagram of results (Not modified, no need to into 4way Eulers)
#+BEGIN_SRC R
