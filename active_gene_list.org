...
set_state <- function(df, statecol, redcol, relcol){
  ...
  df <- df %>%
    mutate(!! statecol := case_when(
                ...
                Variant &
                areaFC < th_areaFC &
                MaxRedPercent < th_redpcnt_dw ~ 'CVG_Repressed_noFC',
                ...
                is.na(MeanPercent) &
                areaFC < th_areaFC &
                MaxRedPercent >= th_redpcnt_up ~ 'Active_RNASeqNA_rescued',
                ...
                Variant &
                is.na(areaFC) &
                MeanRedPercent >= th_redpcnt_dw ~ 'CVG_Active_FCna',

                Variant &
                is.na(areaFC) &
                MeanRedPercent < th_redpcnt_dw ~ 'CVG_Repressed_FCna',
...
## Save results
outname <- sprintf(
  'state_df_rna%s_red_up%s_red_dw%s_reddw%s_areaFC%s_areaFCbig%s_redpcntdif%s.csv',
  th_rnapcnt,
  th_redpcnt_up,
  th_redpcnt_dw,
  th_reddown,
