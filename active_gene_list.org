...
## Select appropiate area for each gene
...
all_df <- select(red_df, Gene_id, contains('Percent')) %>%
  full_join(select(area_df, Gene_id, area_FC, Interval, contains('area')), by = 'Gene_id') %>%
...
set_state <- function(df, outcol, redcol, areacol){
  df <- df %>%
