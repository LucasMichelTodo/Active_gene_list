...
## Here we create a dplyr function. To be able to use variables (for colnames) we need
## to use the special quote functions.
...
set_state <- function(df, statecol, redcol, areacol){
  ...
  df <- df %>%
    mutate(!! statecol := case_when(
                ...
                !! redcol >= th_redrescue ~ 'Var_Active', # noFCrescued

                Variant &
                areaFC >= th_areaFC &
                MaxRedPercentDif >= th_redpcnt &
                !! redcol >= th_redpcnt &
                cut(c(!! areacol, MinArea, MaxArea), 3, labels = labs)[1] == 'max' ~ 'Var_Active',
               
                ## Var repressed
                Variant &
                areaFC < th_areaFC &
                MeanPercent < th_rnapcnt &
                !! redcol < th_redrescue ~ 'Var_Repressed', # noFC
...
set_state(x, state_12B, area_12B) %>%
  print(width = 200)


x = c(2,8,6.1)
cut(x, 3, labels=c('min', 'mid', 'max'))[3] == 'max'
