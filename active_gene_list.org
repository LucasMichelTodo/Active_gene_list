...
set_state <- function(df, outcol, areacol){
  ...
  df <- df %>%
    mutate(!! outcol := case_when(
             ...
             areaFC >= th_areaFC &
             abs(!! areacol - MaxArea) <= abs(!! areacol - MinArea) ~ 'Var_Active',
             
             Variant &
               MaxRedPercentDif >= th_redpcnt &
               areaFC >= th_areaFC &
               abs(!! areacol - MaxArea) > abs(!! areacol - MinArea) ~ 'Var_Respressed',
