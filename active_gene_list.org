...
genestate <- state_df %>%
  mutate(gs_12B = paste(Gene_id, state_12B, sep = "_")) %>%
  mutate(gs_10G = paste(Gene_id, state_10G, sep = "_")) %>%
  mutate(gs_3D7B = paste(Gene_id, state_3D7B, sep = "_")) %>%
  select(Gene_id,
...
x <- makeIntersects(a,b,c)
areas <- lapply(x, function(x) length(x))


f <- euler(c(A=areas$a, B=areas$b, C=areas$c,
        "A&B"=areas$ab, "A&C"=areas$ac, "B&C"=areas$bc,
        "A&B&C" = areas$abc))
