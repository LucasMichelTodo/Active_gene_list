...
excl <- "~/ISGlobal/Variantome_RoviraGraells/3D7_Variantome_AllData_withGam.xls"
...
area_df <- area %>%
  ...
  mutate(MaxArea_12B = apply(select(., contains("12B")), 1, max)) %>%
  mutate(MaxArea_10G = apply(select(., contains("10G")), 1, max)) %>%
  mutate(MaxArea_3D7B = apply(select(., contains("3D7B")), 1, max)) %>%
  mutate(MaxArea = apply(select(., contains('MaxArea_')), 1, max)) %>%
  mutate(MinArea = apply(select(., contains('MaxArea_')), 1, min)) %>%
  mutate(MaxAreaDif = apply(select(., contains('MaxArea_')), 1, max_dif)) %>%
...
1 49.805504 37.95570 25.484634 62.83441 50.462696 37.856349    61.08013
2  3.676218 13.03616  1.789873 10.51234  3.691753  8.610459    12.62246
3        NA       NA 19.333324       NA        NA        NA          NA
4 13.401610 14.78623  7.099032 13.34518 12.041177  8.403034    12.80781
5        NA       NA        NA       NA        NA        NA          NA
6        NA       NA        NA       NA        NA        NA          NA
  MaxArea_10G MaxArea_3D7B  MaxArea  MinArea MaxAreaDif
1    62.38873     62.83441 62.83441 61.08013   1.754283
2    13.03616     10.51234 13.03616 10.51234   2.523820
3          NA           NA       NA       NA         NA
4    14.78623     13.34518 14.78623 12.80781   1.978419
5          NA           NA       NA       NA         NA
6          NA           NA       NA       NA         NA
#+end_example
...
otto <- read_delim("./RNA_Seq_Percentiles/PlasmoDB_Otto.csv", delim=";") %>%
  select(Gene_id = `Gene ID`, MaxPercOtto = `Max %ile (Within Chosen Samples)`)
hoej <- read_delim("./RNA_Seq_Percentiles/PlasmoDB_Hoejimakers.csv", delim=";") %>%
  select(Gene_id = `Gene ID`, MaxPercHoej = `Max %ile (Within Chosen Samples)`)
toen <- read_delim("./RNA_Seq_Percentiles/PlasmoDB_Toenke.csv", delim=";") %>%
  select(Gene_id = `Gene ID`, MaxPercToen = `Max %ile (Within Chosen Samples)`)
bart <- read_delim("./RNA_Seq_Percentiles/PlasmoDB_Bartfai.csv", delim=";") %>%
...
all_df <- select(red_df, Gene_id, contains('Percent')) %>%
  full_join(select(area_df,
                   Gene_id,
                   contains('MaxArea_'),
                   MaxArea,
                   MinArea,
                   MaxAreaDif), by = 'Gene_id') %>%
  full_join(select(rna_df, Gene_id, MeanPercent), by = 'Gene_id')

## Add Vartiant Genes information

cvg <- read_excel("./CVG_list_jan2020_final.xlsx", sheet = "Final")
