...
set_state <- function(df, statecol, redcol, relcol){
  ...
  df <- df %>%
    mutate(!! statecol := case_when(
                ...
                MaxRedPercent >= th_redpcnt  ~ 'CVG_Semiactive_noFC',

                Variant &
                areaFC < th_areaFC &
                MaxRedPercent < th_redpcnt ~ 'CVG_Repressed_noFC',
                ...
                MaxRedPercent < th_reddown &
                !! relcol == 'max' ~ 'CVG_Semiactive_FC_reddown',

                Variant &
                areaFC >= th_areaFC_redrescue &
                MaxRedPercent < th_reddown &
                !! relcol == 'mid' ~ 'CVG_Repressed_FC_reddown',

                Variant &
                areaFC >= th_areaFC_redrescue &
                MaxRedPercent < th_reddown &
                !! relcol == 'min' ~ 'CVG_Repressed_FC_reddown',
...
## Save results
outname <- sprintf(
  'state_df_rna%s_red%s_redup%s_redresc%s_reddw%s_areaFC%s_areaFCbig%s_redpcntdif%s.csv',
  th_rnapcnt,
  th_redpcnt,
  th_redpcnt_up,
  th_redrescue,
...
vi_12B <- state_df %>%
  filter(category_12B == "CVG_Repressed") %>%
  select(Gene_id) %>%
  pull()

vi_10G <- state_df %>%
  filter(category_10G == "CVG_Repressed") %>%
  select(Gene_id) %>%
  pull()

vi_3D7B <- state_df %>%
  filter(category_3D7B == "CVG_Repressed") %>%
  select(Gene_id) %>%
  pull()


customEuler(vi_12B, vi_10G, vi_3D7B, 'CVG_Repressed_Genes')
